...
export function Topbar({ profile, title, description, demoMode }: TopbarProps) {
  return (
    <header className="flex flex-col gap-4 rounded-[30px] border border-slate-200/80 bg-white p-6 shadow-panel md:flex-row md:items-start md:justify-between">
      <div>
        <div className="mb-2 flex items-center gap-3">
          <StatusBadge variant={demoMode ? "warning" : "info"}>
            {demoMode ? "Demo modus" : profile.role}
          </StatusBadge>
      ...
      <div className="flex items-center gap-3">
        ...
        <SignOutButton demoMode={demoMode} />
      </div>
    </header>
  );
}
